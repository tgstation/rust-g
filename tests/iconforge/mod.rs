...
use std::{fs::{read_dir, File}, io::BufReader, path::Path};
use dmi::{error::DmiError, icon::{Icon, IconState}};
        ...
        if let Ok(entry) = tmp_entry {
            if let Some(file_name) = entry.file_name().to_str() {
                ...
                if let Some(diff) = compare_dmis(entry.path().as_path(), rustg_path) {
                    differences.push(format!("icon {} differs from {}:\n{}", rustg_path.display(), entry.path().display(), diff));
                }
            }
        }
    }
    if !differences.is_empty() {
       panic!("icons were found to differ:\n\n---\n{}", differences.join("\n\n---\n"))
    } else {
...
fn compare_dmis(dm_path: &Path, rustg_path: &Path) -> Option<String> {
    println!("Comparing {} and {}", dm_path.display(), rustg_path.display());
    let mut differences: Vec<String> = Vec::new();
    let dm_icon = dmi_from_path(dm_path).unwrap();
    let rustg_icon = dmi_from_path(rustg_path).unwrap();
    for dm_state in &dm_icon.states {
        if let Some(rustg_state) = rustg_icon.states.iter().find(|rustg_state| rustg_state.name == dm_state.name) {
    ...
    for rustg_state in &rustg_icon.states {
        if let None = dm_icon.states.iter().find(|dm_state| dm_state.name == rustg_state.name) {
            differences.push(format!("icon state {}:\ndoes not exist in dm", rustg_state.name));
        }
    }
    if differences.is_empty() {
        None
...
fn compare_states(dm_state: &IconState, rustg_state: &IconState) -> Option<String> {
    ...
    if dm_state.dirs != rustg_state.dirs {
        differences.push(format!("DIRS: dm: {} - rustg: {}", dm_state.dirs, rustg_state.dirs));
    }

    if dm_state.frames != rustg_state.frames {
        differences.push(format!("FRAMES: dm: {} - rustg: {}", dm_state.frames, rustg_state.frames));
    }

    if dm_state.movement != rustg_state.movement {
        differences.push(format!("MOVEMENT FLAG: dm: {} - rustg: {}", dm_state.movement, rustg_state.movement));
    }

    if dm_state.rewind != rustg_state.rewind {
        differences.push(format!("REWING FLAG: dm: {} - rustg: {}", dm_state.rewind, rustg_state.rewind));
    }

    if dm_state.loop_flag != rustg_state.loop_flag {
        differences.push(format!("LOOP FLAG: dm: {:?} - rustg: {:?}", dm_state.loop_flag, rustg_state.loop_flag));
    }

    let dm_images_len = dm_state.images.len();
    let rustg_images_len = rustg_state.images.len();
    if dm_images_len != rustg_images_len {
        differences.push(format!("IMAGE COUNT: dm: {} - rustg: {}", dm_images_len, rustg_images_len));
    } else {
        compare_images(&mut differences, &dm_state.images, &rustg_state.images, dm_state.dirs);
    }
...
fn compare_images(differences: &mut Vec<String>, dm_images: &Vec<DynamicImage>, rustg_images: &Vec<DynamicImage>, dirs: u8) {
    ...
    for (dm_image, rustg_image) in std::iter::zip(dm_images, rustg_images) {
        ...
        if !image_differences.is_empty() {
            ...
            differences.push(format!("{} at pixels: {all_coordinates}", image_name_from_index(image_index, dirs)));
        }
        image_index += 1;
    }
}
