...
byond_fn!(fn iconforge_generate(file_path, spritesheet_name, sprites, hash_icons, generate_dmi, flatten) {
    ...
    let result = Some(match catch_panic(|| generate_spritesheet(&file_path, &spritesheet_name, &sprites, &hash_icons, &generate_dmi, &flatten)) {
        Ok(o) => o.to_string(),
        Err(e) => e.to_string()
    });
    frame!();
...
byond_fn!(fn iconforge_generate_async(file_path, spritesheet_name, sprites, hash_icons, generate_dmi, flatten) {
    ...
    Some(jobs::start(move || {
        let result = match catch_panic(|| generate_spritesheet(&file_path, &spritesheet_name, &sprites, &hash_icons, &generate_dmi, &flatten)) {
            Ok(o) => o.to_string(),
            Err(e) => e.to_string()
        };
        frame!();
